...
| image.digest | string | `"sha256:97e7cf879bc47b3cec6206b79ab8604d05d1058c4a7398ff29aedacf49524715"` | Image digest (`sha256:...`). When set it is appended to the tag, which pins the deployment to one immutable build even though the homelab repo only ever tracks a chart version. Rewritten by the release workflow. |
| image.pullPolicy | string | `"IfNotPresent"` | Image pull policy. |
| image.repository | string | `"ghcr.io/wihrt/blog"` | Image repository. |
| image.tag | string | `"0.1.2"` | Image tag. Rewritten by the release workflow; defaults to `.Chart.AppVersion`. |
