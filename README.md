...
There is no `CHANGELOG.md`. Nothing in the pipeline pushes to `main`, so a
committed changelog would have to be maintained by hand and would rot. Each
GitHub Release carries the conventional-commit changelog for its version,
generated by `cog`, alongside the image and chart references it published.

`mise run release:next` shows the version the next release would produce.
